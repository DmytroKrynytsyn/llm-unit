...
There's no in-cluster pull or download. `LLAMA_MODEL_PATH` (mapped from `Values.llamacpp.modelHostPath`, a single `.gguf` file) and `LLAMA_BIN_DIR` (mapped from `Values.llamacpp.binDir`, normally `/opt/ollama-build/latest`) are both `hostPath` mounts onto files/dirs placed on the node by hand — switching models means copying a new `.gguf` file onto the node yourself and updating `llamacpp.modelHostPath`, not just rolling the DaemonSet. On startup the broker launches `llama-server -m $LLAMA_MODEL_PATH --port $LLAMA_PORT` as a subprocess bound to `127.0.0.1` and polls its `/health` endpoint before connecting to RabbitMQ. FastAPI's startup event blocks request handling until that subprocess reports healthy, so `/health` won't go green until the model is loaded. The broker's own `/health` also reflects whether the `llama-server` subprocess is still alive — if it dies, the liveness probe fails and the whole pod (and subprocess with it) restarts.
